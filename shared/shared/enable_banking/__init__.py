"""Shared Enable Banking API client.
...
class EnableBankingClient:
    ...
    def start_authorization(
        ...
        try:
            ...
            if "REDIRECT_URI_NOT_ALLOWED" in str(exc):
                raise EnableBankingError(
                    f"{exc} — redirect_url {redirect_url!r} must exactly match a "
                    "URL registered for this app in the Enable Banking Control Panel "
                    "(https://enablebanking.com/cp/applications), including scheme "
                    "and trailing slash."
                ) from exc
            raise
    ...
    def get_account_balances(self, account_uid: str) -> list[dict[str, Any]]:
        """Return balance objects from GET /accounts/{uid}/balances."""
