"""Convert Lloyds / Bank of Scotland CSV exports into hub JSON (same outputs as Excel)."""
...
def read_csv_rows(text: str) -> list[dict[str, str]]:
    """Parse BoS/Lloyds CSV text into row dicts keyed by normalized header names."""
    if not text.strip():
        return []
    sample = text[:4096]
    try:
    ...
    missing = BOS_LLOYDS_HEADERS - set(field_map)
    if missing:
        raise ValueError(
            f"Not a BoS/Lloyds CSV — missing columns: {', '.join(sorted(missing))}"
        )
...
def is_bos_lloyds_csv_bytes(data: bytes) -> bool:
    try:
...
def parse_bos_lloyds_date(value: str) -> str | None:
    """``22/06/2026`` → ``22-06-2026`` (app date format)."""
...
def first_entry_year(rows: list[dict[str, str]]) -> str | None:
    for row in rows:
        parsed = parse_bos_lloyds_date(row.get("transaction date", ""))
        if not parsed:
            continue
        parts = parsed.split("-")
...
def _clean_sort_code(value: str) -> str:
    text = str(value or "").strip().lstrip("'").replace("-", "")
    return text


def _row_amount_cents(row: dict[str, str]) -> int:
    debit = str(row.get("debit amount") or "").strip()
    credit = str(row.get("credit amount") or "").strip()
    if credit:
        return cell_amount_cents(credit)
    if debit:
        return -cell_amount_cents(debit)
    return 0
...
def _latest_balance_cents(rows: list[dict[str, str]]) -> int | None:
    ...
    for row in rows:
        date = parse_bos_lloyds_date(row.get("transaction date", ""))
        if not date:
            continue
...
def _account_holder_from_rows(rows: list[dict[str, str]]) -> tuple[str, str]:
    for row in rows:
        sort_code = _clean_sort_code(row.get("sort code", ""))
        number = str(row.get("account number") or "").strip()
        if sort_code or number:
            label = f"{sort_code}/{number}".strip("/") if sort_code else number
            return label or "onbekend", label or "onbekend"
    ...
    *,
    ...
    for row in rows:
        ...
        if not account_ref:
            sort_code = _clean_sort_code(row.get("sort code", ""))
            number = str(row.get("account number") or "").strip()
            account_ref = f"{sort_code}/{number}".strip("/") if sort_code else number
        description = str(row.get("transaction description") or "").strip()
        type_code = str(row.get("transaction type") or "").strip()
...
def import_person_debit_credit_csv(
    *, data_dir: Path, categories_path: Path, tx_type: str = TX_TYPE_DEFAULT
) -> dict[str, Any]:
    """Hub upload / refresh entry: rewrite JSON from ``*.csv`` (Lloyds/BoS layout)."""
...
def import_person_bos_lloyds_csv(*, data_dir: Path, categories_path: Path) -> dict[str, Any]:
    """Back-compat alias."""
    return import_person_debit_credit_csv(
        data_dir=data_dir, categories_path=categories_path, tx_type="LLOYDS-csv"
    )
