...
# personal | local | regional_admin | <legacy country from file config>
...
def normalize_access(raw: str | None) -> str:
    """Map legacy aliases; leave country names as-is (file config only)."""
    mode = str(raw or ACCESS_LOCAL).strip().lower()
    if mode in ("central", "regional"):
        return ACCESS_REGIONAL_ADMIN
    return mode or ACCESS_LOCAL


def is_country_access(mode: str | None = None) -> bool:
    """True when access is a legacy country key (file config only)."""
    value = normalize_access(mode if mode is not None else access_mode())
    return value not in (ACCESS_REGIONAL_ADMIN, ACCESS_LOCAL, ACCESS_PERSONAL)
...
def is_regional_admin() -> bool:
    ...
    mode = access_mode()
    if mode in (ACCESS_PERSONAL, ACCESS_LOCAL):
        return False
    if mode == ACCESS_REGIONAL_ADMIN:
        return True
    return is_country_access(mode)
