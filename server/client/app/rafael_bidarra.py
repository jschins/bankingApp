"""Convert Rafael Bidarra trimester Excel files to hub JSON format."""
...
def person_root() -> Path:
    """Directory containing rafael_bidarra.exe (or this module in dev)."""
    if getattr(sys, "frozen", False):
        return Path(sys.executable).resolve().parent
    return Path(__file__).resolve().parents[2] / "workspaces" / "dkg" / "rafael_bidarra"
...
def _category_code(value: str | None) -> int | None:
    ...
    try:
        code = int(float(text))
        # Rafael mapping: category 3 is treated as category 18.
        return 18 if code == 3 else code
    except ValueError:
        return None
