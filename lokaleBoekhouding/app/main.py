...
async def lifespan(_app: FastAPI):
    from app.centrale_sync import login_and_pull, logout_and_push
    ...
    if not pull.get("ok"):
        ...
        print(f"centrale login+pull ok (workspace={pull.get('workspace')})")
    init_app()
    try:
        yield
    finally:
        push = logout_and_push()
        if not push.get("ok"):
            print(f"WARNING: centrale push/logout failed: {push.get('error')}")
        elif not push.get("skipped"):
            print(f"centrale logout+push ok (workspace={push.get('workspace')})")


app = FastAPI(title="lokaleBoekhouding", version="0.1", lifespan=lifespan)
...
def health() -> dict[str, Any]:
    from app.centrale_sync import load_config, lock_status
    ...
    return {
        ...
        "centrale": {
            ...
            "enabled": cfg.enabled,
            **{k: v for k, v in lock_status().items() if k not in ("workspace", "centrale_url", "enabled")},
        },
        "people": [
...
@app.get("/api/centrale/lock")
def api_centrale_lock() -> dict[str, Any]:
    from app.centrale_sync import lock_status

    return lock_status()


@app.post("/api/centrale/logout")
def api_centrale_logout() -> dict[str, Any]:
    """Push selected files to centrale and clear the local overwrite-warning sticky state."""
    from app.centrale_sync import logout_and_push

    result = logout_and_push()
    if not result.get("ok") and not result.get("skipped"):
        raise HTTPException(status_code=502, detail=result.get("error") or "logout failed")
    return result
...
def api_refresh(body: RefreshRequest | None = None) -> dict[str, Any]:
    ...
    return refresh_all(date_from=req.date_from, date_to=req.date_to)
